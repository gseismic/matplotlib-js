...
// import { MouseEventHandler } from '../utils/signal.js';
...
class Figure extends Canvas {
              ...
              wspace: number = 20, hspace: number = 20): Axes 
  {
    ...
    this.scenes.sort((a, b) => a.z_index - b.z_index);
  ...
  draw() {
    ...
    for (const scene of this.scenes) {
      scene.render(this.renderer, false);
    }
    this.renderer.flush();
  }
  ...
  on_mouse_move(position: Point) {
    console.log(position);
  }

  on_mouse_click(position: Point) {
    console.log(position);
  }
}
